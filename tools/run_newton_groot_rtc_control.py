...
from rexpolicy.camera_preprocessing import preprocess_ego_rgb  # noqa: E402
from rexpolicy.ik import (  # noqa: E402
    FullPoseDifferentialIkController,
    FullPoseDifferentialIkControllerConfig,
    RobotStateSnapshot,
    TaskSpaceTarget,
)
from rexpolicy.ik.nero_can_fk import nero_can_flange_pose_from_joints  # noqa: E402
from rexpolicy.models import NamedJointValues, Pose7  # noqa: E402
from rexpolicy.policies.groot_rotation_contract import (  # noqa: E402
    row_first_rot6d_to_matrix,
)
from rexpolicy.retargeting.hand_config import load_linker_l10_right_hand_spec  # noqa: E402
from rexpolicy.robots.newton_runtime import NewtonLinkKinematicsModel  # noqa: E402
from rexpolicy.teleop.spatial_frames import matrix_to_quat_xyzw, quat_xyzw_to_matrix  # noqa: E402


def _resolve_external_default(env_name: str, *candidates: Path) -> Path:
...
DEFAULT_ISAAC_GROOT_ROOT = _resolve_external_default(
    "ISAAC_GROOT_ROOT",
    REPO_ROOT / "external" / "Isaac-GR00T",
    REPO_ROOT.parent / "Isaac-GR00T",
)
DEFAULT_POLICY_CHECKPOINT = _resolve_external_default(
    "GROOT_POLICY_CHECKPOINT",
    REPO_ROOT / "checkpoints" / "groot" / "checkpoint-200000",
    REPO_ROOT.parent / "newton" / "checkpoints" / "groot" / "checkpoint-200000",
)
DEFAULT_VLM_MODEL = _resolve_external_default(
    "GROOT_VLM_MODEL",
    REPO_ROOT / "checkpoints" / "nvidia" / "Cosmos-Reason2-2B",
    REPO_ROOT.parent / "newton" / "checkpoints" / "nvidia" / "Cosmos-Reason2-2B",
)
DEFAULT_SMOOTH_DIR = _resolve_external_default(
    "GROOT_SMOOTH_DATASET",
    REPO_ROOT / "local_data" / "groot" / "smooth",
    REPO_ROOT.parent / "newton" / "local_data" / "groot" / "smooth",
)
...
def _validate_asset_layout(args: argparse.Namespace) -> CheckpointModalities:
    ...
    if args.image_source == "smooth" or args.state_source == "smooth" or args.validate_only:
...
class GrootRtcExample(scene_runtime.Example):
    def __init__(self, viewer: Any, args: argparse.Namespace) -> None:
        ...
        uses_smooth_data = self.image_source == "smooth" or self.state_source == "smooth"
        self.smooth = (
            SmoothEpisodeSource(args.smooth_dir, args.episode_index, loop=args.smooth_loop)
            if uses_smooth_data
            else None
        )
        self.instruction = str(args.instruction).strip() or (
            self.smooth.task if self.smooth is not None else DEFAULT_INSTRUCTION
        )
        ...
        print(
            ...
            f"episode={self.smooth.episode_index if self.smooth is not None else 'sim-only'} "
    ...
    def _build_observation(self) -> tuple[dict[str, Any], dict[str, Any]]:
        ...
        if self.image_source == "smooth":
            assert self.smooth is not None
        ...
        metadata = {
            ...
            "episode_index": self.smooth.episode_index if self.smooth is not None else None,
            "smooth_frame": self.smooth._frame_index(smooth_frame) if self.smooth is not None else None,
...
def _add_policy_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        help="Instruction for simulator observations. Empty uses the selected smooth episode task when available.",
    )
